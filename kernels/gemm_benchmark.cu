...
template<int T, int Ts> // (T, T): Number of threads per block,  (Ts, Ts): Number of cells each thread owns (or accumulates)
__global__ void regtiled_gemm_vecloaded(float *A, float *B, float* C, int M, int N, int K) {
...
int main() {
    ...
    // --- Benchmark on large matrix ---
    {
        ...
        float ms_regtiled_vecloaded = time_kernel([&]{regtiled_gemm_vecloaded<T_regtiled, Ts_regtiled><<<blocks_regtiled, threads_regtiled>>>(dA, dB, dC, M, N, K); });
        ...
        printf("%-20s %8.3f ms  %8.1f GFLOPS  (%.1f%% of peak)\n", "regtiled_vecloaded", ms_regtiled_vecloaded, flops/(ms_regtiled_vecloaded*1e6), flops/(ms_regtiled_vecloaded*1e6)/91600*100);
